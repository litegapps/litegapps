...
#litegapps props
litegapps.type=607060
litegapps.apk.compress=607070
#path
...
#### Diference litegapps++
if [ "$litegapps.type" -eq 607060 ]; then
	sedlog "litegapps type : litegapps"
elif [ "$litegapps.type" = 607060 ]; then
	sedlog "litegapps type : litegapps++"
	litegapps_plus
else
	sedlog "litegapps type code does not match it is automatically returned to type : litegapps"
fi
...
if [ "$litegapps.apk.compress" -eq 607070 ]; then
	sedlog "Using litegapps system compress apk"
	litegapps_compress_system
else
