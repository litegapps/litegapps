...
make_flashable_litegapps(){
	for WFL in MAGISK RECOVERY AUTO; do
		...
		if [ -d $BASED/modules/$W_ARCH/$W_SDK ]; then
			...
			local MODULE_DESC="LiteGapps Core is a custom gapps which provides the necessary packages for gapps."
			...
			local NAME_ZIP="[$WFL]LiteGapps_Core_${W_ARCH}_$(get_android_version $W_SDK)_v${PROP_VERSION}_${PROP_STATUS}.zip"
			local OUT_ZIP=$out/litegapps/$W_ARCH/$W_SDK/core
			printlog "- Build ZIP"
			cd $tmp/$WFL
