...
for CCACHE in $LITEGAPPS/log $TMPDIR; do
...
#checking sdk files
if [ ! -d $TMPDIR/$ARCH/$SDKTARGET ]; then
...
if [ -d $TMPDIR/$ARCH/$SDKTARGET/system ]; then
	sedlog "- Copying system"
	listlog $TMPDIR
	cp -af $TMPDIR/$ARCH/$SDKTARGET/system/* $sysdirtarget/
fi

if [ -d $TMPDIR/$ARCH/$SDKTARGET/vendor ]; then
	sedlog "- Copying vendor"
	listlog $TMPDIR
	cp -af $TMPDIR/$ARCH/$SDKTARGET/vendor/* $vendirtarget/
fi
