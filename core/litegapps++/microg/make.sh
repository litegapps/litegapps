...
make_flashable_litegapps(){
	for WFL in MAGISK RECOVERY AUTO; do
		...
		#fi
			...
			local MODULE_DESC="litegapps++ MicroG is a google apps package that supports almost all arch and android versions while maintaining lightness, saving battery and more."
			sed -i 's/'"$(getp litegapps_type $MODULE_PROP)"'/'"litegapps_plus"'/g' $MODULE_PROP
			sed -i 's/'"$(getp litegapps_apk_compress $MODULE_PROP)"'/'"${apk_compessed_type}"'/g' $MODULE_PROP
			sed -i 's/'"$(getp name $MODULE_PROP)"'/'"LiteGapps++ MicroG $PROP_STATUS"'/g' $MODULE_PROP
			...
			local NAME_ZIP="[$WFL]LiteGapps++_MicroG_v${PROP_VERSION}_${PROP_STATUS}.zip"
			local OUT_ZIP=$out/litegapps++/microg
			printlog "- Build ZIP"
			cd $tmp/$WFL
...
BIN_ARCH=arm
clear
sedlog "Building LiteGapps++ MicroG"
printmid "Building LiteGapps++ MicroG"
printlog " "
printlog " "
