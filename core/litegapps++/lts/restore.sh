...
for WAHYU in sdk cross_system arch; do
	if [ -f $GAPPS_FILES/$WAHYU.zip ]; then
       ...
       curl -L -o $GAPPS_FILES/$WAHYU.zip https://gitlab.com/litegapps/litegapps-server/-/raw/main/litegapps++/lts/$WAHYU.zip >/dev/null 2>&1
       if [  $? -eq 0 ]; then
