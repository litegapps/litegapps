...
make_flashable_litegapps(){
	for WFL in MAGISK RECOVERY AUTO; do
		...
		#fi
			...
			local MODULE_DESC="litegapps++ LTS is a google apps package that supports almost all arch and android versions while maintaining lightness, saving battery and more."
			...
			sed -i 's/'"$(getp name $MODULE_PROP)"'/'"LiteGapps++ LTS $PROP_STATUS"'/g' $MODULE_PROP
			...
			local NAME_ZIP="[$WFL]LiteGapps++_LTS_v${PROP_VERSION}_${PROP_STATUS}.zip"
			local OUT_ZIP=$out/litegapps++/LTS
			printlog "- Build ZIP"
			cd $tmp/$WFL
...
BIN_ARCH=arm
clear
sedlog "Building LiteGapps++ LTS"
printmid "Building LiteGapps++ LTS"
printlog " "
printlog " "
